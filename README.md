...
- ⚙️ **管理面板**：聊天室弹窗 + 独立页面，二级跳转布局（一级横向滚动菜单 → 二级分区内容 + 返回），复古终端风格（L 形四角开合动画）；管理用户 / 权限组 / 插件 / 文件上传 / 流量 / 数据库 / 设置；全部对话框为自研组件（焦点圈闭、ESC 关闭、动态内容零 HTML 注入）
- 🔌 **插件系统**：文件夹式与单文件式插件，支持命令、钩子、页面、CSS/JS 注入
- 📎 **文件上传**：自动识别图片、音频、普通文件（`::img::`、`::wav::`、`::file::` 标记）；多文件批量上传——拖拽/选择**追加并去重**（上限 20 个），发送时逐文件进度条；管理面板"上传"页支持批量上传与服务器文件管理
...
- 📱 **手机端适配**：窄屏（≤768px）下所有弹窗（管理面板 / 外观设置 / 文件预览等）自动全屏页面化、关闭按钮加大触控目标，页面悬浮按钮不穿透遮挡
...
| `admin.tools` / `admin.uploads` | 快捷工具链接管理 / 上传文件管理 |
...
- **权限组**：增删组、勾选权限点、设置默认组
- **插件**：启停、重载、编辑 JSON 配置
- **上传**：多文件批量上传（点击/拖拽入队、去重、单次最多 20 个），逐文件进度与总进度条；管理 `static/uploads` 已有文件（复制链接、删除；需 `admin.uploads` 权限）
- **流量**：总请求数、今日请求、独立 IP、近 7 天柱状图（生长动画）、热门路径
...
- **二级跳转 + 复古终端风**：一级为横向可滚动菜单（通高倾斜矩形按钮，rotate 保持直角、独立色描边、hover 自右向左纯色填充 + 发光），二级为分区内容 + 返回按钮；直角窗口、等宽字体、错峰入场；面板全域滚动条主题化美化
- **自研对话框**：所有确认/输入/提示均为自制弹窗（替代原生 `prompt`/`confirm`/`alert`），支持 ESC / 遮罩关闭、Tab 焦点圈闭、输入校验；动态内容全部经 `textContent` 写入，杜绝 HTML 注入
- **登录失效**：确认弹窗之上叠加全屏故障动画（扫描噪声 + 横向撕裂条，覆盖包括弹窗在内的整个屏幕）
...
- "外观设置"窗口菜单与管理面板按钮同款：倾斜矩形、独立色描边、hover 自右向左纯色填充 + 发光
- 页面元素错峰出场动画；登录失效时全屏故障动画；管理面板窗口 L 形四角开合、彩色导航填充动效、柱状图生长、toast 滑入滑出；所有动画尊重 `prefers-reduced-motion`
