...
>
> **add_tool_link 规则**：以 `/` 开头的地址一律按**根路径**定位（与插件绝对注册一致），不再自动补 `base_path` 前缀（如配置了 `/chat` 挂载路径时 `/about` 仍是 `/about`）。插件提供的链接地址建议使用绝对注册的蓝图地址（如 `/plugins/hello_world/about`）。
...
> **注意**：Flask 在应用处理首个请求后禁止注册新蓝图。运行中热加载/热启动的插件若含蓝图，蓝图不会生效（重载时自动以唯一名重试注册，仅限首个请求前），需重启服务；聊天命令、钩子、CSS/JS 注入、快捷工具链接则即时生效。
...
- 管理面板"插件"标签页可查看每个插件的启用状态与加载状态；启用/禁用即时热生效（停止时命令/钩子/注入全部卸载），修改代码后可对单个插件"重载"或"重载全部"，新增插件文件无需重启即可"启用"加载。
...
- 蓝图注册需在首个请求前完成；运行中热停止无法注销已注册的路由，热重载仅对命令/钩子/注入/工具链接生效。
